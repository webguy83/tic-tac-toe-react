...
interface MainMenuProps {
  startGame: (mode: 'cpu' | 'player') => void;
}

const MainMenu: React.FC<MainMenuProps> = ({ startGame }) => {
  ...
  return (
    <div className='main-menu'>
      ...
      <button type='button' className='btn primary' onClick={() => startGame('cpu')}>
        New Game (VS CPU)
      </button>
      <button type='button' className='btn secondary' onClick={() => startGame('player')}>
        New Game (VS Player)
      </button>
    </div>
  );
